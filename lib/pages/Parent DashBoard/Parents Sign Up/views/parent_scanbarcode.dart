...
import 'package:school_management/pages/Parent%20DashBoard/Parents%20Sign%20Up/views/parent_input_field1.dart';
...
class ParentsScanCode extends StatefulWidget {
  const ParentsScanCode({super.key});

  @override
  State<ParentsScanCode> createState() => _ParentsScanCodeState();
}

class _ParentsScanCodeState extends State<ParentsScanCode> {
  ...
  popAuth(String? result) async {
    regController.parentAdminId.value = result!;
    debugPrint(regController.parentAdminId.value);
    showAuthLoading("Checking your Qrcode");

    regController.schoolModel.value =
        await ApiCalls().getSchoolDetails(regController.parentAdminId.value);
    if (regController.schoolModel.value != null) {
      showSuccess();
      await Future.delayed(const Duration(seconds: 1), () {});
      Get.to(() => ParentsRegistrationScreen());
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Center(
                  child: Text(
                "To confirm your identity as a Parent of a school, please scan the QR code on the phone of your school administrator",
                textAlign: TextAlign.center,
                style: TextStyle(
                    color: maincolor,
